...
function AntiTamper:apply(ast, pipeline)
    ...
    if self.UseDebug then
        ...
        code = code .. [[pcall(function()getgenv()["25ms was here :)"]=function()end end)
        ]]
    end
    code = code .. [[
    local gmatch = string.gmatch;
    local err = function() error("I meow a lot!") end;
    ...
    local meow=true
    if load then
        local l1=tonumber(gmatch(tostring(({load('return pcall(function()return 1/"abc"end)')()})[2]), ':(%d*):')());
        local l2=tonumber(gmatch(tostring(({load('\nreturn pcall(function()return 1/"abc"end)')()})[2]), ':(%d*):')());
        meow=l1~=l2
    end

    valid = valid and acc1 == acc2 and meow;
    local _=not valid and err()
