...
function Unparser:unparseStatement(statement, tabbing)
	...
	elseif(statement.kind == AstKind.WhileStatement) then
		...
		code = "while _25msWHILE(" .. expressionCode 
	...
	elseif(statement.kind == AstKind.RepeatStatement) then
		...
		code = "repeat" ..  self:whitespaceIfNeeded(bodyCode, self:newline(true))
			.. bodyCode
			.. self:whitespaceIfNeeded2(bodyCode, self:newline() .. self:tabs(tabbing, true)) .. "until _25msREPEAT(" .. expressionCode..")";
	...
	elseif(statement.kind == AstKind.IfStatement) then
		...
		local bodyCode = self:unparseBlock(statement.body, tabbing);
		code = "if " .."_25msIF(".. exprcode .. ")".. " then" .. self:whitespaceIfNeeded(bodyCode, self:newline(true))
			.. bodyCode;
		...
		for i, eif in ipairs(statement.elseifs) do
			...
			code = code .. self:newline(false) .. self:whitespaceIfNeeded2(code, self:tabs(tabbing, true)) .. "elseif " .."_25msIF(".. exprcode .. ")".. " then"
...
local function addRandomUnderscores(str)
	local t = {}
	for i = 1, #str do
		table.insert(t, str:sub(i, i))
		-- Randomly add an underscore with 50% chance
...
local function customescape(str)
	return str:gsub(".", function(c) return ((c=="\n" or c=="\\" or c=='"' or c:byte() > 125 or c:byte() < 33) and util.escape(c) or c) end)
end
...
function Unparser:unparseExpression(expression, tabbing)
	...
	if expression.kind == AstKind.NumberExpression then
		local num = expression.value
		local str = tostring(expression.value)
		if str == "inf" then
			return "2e1024"
		elseif str == "-inf" then
			return "-2e1024"
		elseif str:sub(1, 2) == "0." then
			str = str:sub(2)
		end
		return str
	end
	...
	if(expression.kind == AstKind.StringExpression) then
		return "\"" .. customescape(expression.value) .. "\"";
	end
	...
	if(expression.kind == k) then
		...
		return "_25msOR(".. lhs .. ",function()return(" .. rhs.. ")end)";
	end
	...
	if(expression.kind == k) then
        ...
        return "_25msAND(".. lhs .. ",function()return(" .. rhs.. ")end)";
	end
	...
	if(expression.kind == k) then
		...
		return "_25msLE("..lhs .. self:optionalWhitespace() .. "," .. self:optionalWhitespace() .. rhs..")";
	end
	...
	if(expression.kind == k) then
		...
		return "_25msGR("..lhs .. self:optionalWhitespace() .. "," .. self:optionalWhitespace() .. rhs..")";
	end
	...
	if(expression.kind == k) then
		...
		return "_25msLEEQ("..lhs .. self:optionalWhitespace() .. "," .. self:optionalWhitespace() .. rhs..")";
	end
	...
	if(expression.kind == k) then
		...
		return "_25msGREQ("..lhs .. self:optionalWhitespace() .. "," .. self:optionalWhitespace() .. rhs..")";
	end
	...
	if(expression.kind == k) then
		...
		return "_25msUNEQ("..lhs .. self:optionalWhitespace() .. "," .. self:optionalWhitespace() .. rhs..")";
	end
	...
	if(expression.kind == k) then
		...
		return "_25msEQ("..lhs .. self:optionalWhitespace() .. "," .. self:optionalWhitespace() .. rhs..")";
	end
